...
{  
  ...
  parse("KCUT",kcut); // read knn parameter (Z)
  ...
  // inputs.push_back( single_input4 );
  ...
  atomsToRequest.reserve ( atom_lista.size() ); 
  ...
  for(unsigned j=0; j<_n_out; j++) {
    ...
    for(unsigned i=rank;i<nat;i+=stride) {
	  for(unsigned k=0; k<kcut; k++) {
		...
		deriv[i] += dist_deriv_mat[i][k]*der[i*kcut+k];
        deriv[idx] += -1.0*dist_deriv_mat[i][k]*der[i*kcut+k];
	  }
	}
	if(!serial) {
    ...
    for(unsigned k=0; k<nat; k++) {
      setAtomsDerivatives( getPntrToComponent(name_comp), k, deriv[k] );
    }
  }
